...
def test_update_title(db: Connection[DictCursor], term_id: str) -> None:
    ...
    with db.cursor() as cursor:
        cursor.execute(
            "UPDATE terms SET title = %s WHERE id = %s",
            ("Spring 2026", term_id),
        )
...
def test_update_start_date(db: Connection[DictCursor], term_id: str) -> None:
    ...
    with db.cursor() as cursor:
        cursor.execute(
            "UPDATE terms SET start_date = %s WHERE id = %s",
            ("2025-09-01", term_id),
        )
...
def test_update_end_date(db: Connection[DictCursor], term_id: str) -> None:
    ...
    with db.cursor() as cursor:
        cursor.execute(
            "UPDATE terms SET end_date = %s WHERE id = %s",
            ("2025-12-20", term_id),
        )
...
def test_update_active(db: Connection[DictCursor], term_id: str) -> None:
    ...
    with db.cursor() as cursor:
        cursor.execute(
            "UPDATE terms SET active = %s WHERE id = %s", (True, term_id)
        )
...
def test_insert_duplicate_title_raises(
    db: Connection[DictCursor], term_id: str
) -> None:
    insert_term(db, term_id, title="Fall 2025")
...
def test_insert_leap_year_date(
    db: Connection[DictCursor], term_id: str
) -> None:
...
def test_insert_same_day_start_and_end(
    db: Connection[DictCursor], term_id: str
) -> None:
...
def test_find_all_returns_two_inserted_terms(
    db: Connection[DictCursor], term_id: str
) -> None:
    id2 = str(uuid.uuid4())
    insert_term(db, term_id, title="Fall 2025")
