    ...
    try:
        with db.cursor() as cursor:
            cursor.execute(
                ...
                (term.id, term.title, term.start_date, term.end_date, term.active)
            )
        db.commit()
    except pymysql.IntegrityError:
        raise HTTPException(status_code=409, detail="A term with this title already exists")
    return term

@router.put("/{term_id}", response_model=TermResponse)
def update_term(term_id: str, term: TermCreate, db: Connection = Depends(get_db)):
    try:
        with db.cursor() as cursor:
            cursor.execute(
                "UPDATE terms SET title = %s, start_date = %s, end_date = %s, active = %s WHERE id = %s",
                (term.title, term.start_date, term.end_date, term.active, term_id)
            )
        db.commit()
    except pymysql.IntegrityError:
        raise HTTPException(status_code=409, detail="A term with this title already exists")
