...
def get_assignments_by_course(
    course_id: str, db: Connection = Depends(get_db)
):
    with db.cursor() as cursor:
        cursor.execute(
            "SELECT id, course_id, title, description, category, due_date, "
...
def create_assignment(
    assignment: AssignmentCreate, db: Connection = Depends(get_db)
):
    try:
        with db.cursor() as cursor:
            cursor.execute(
...
def update_assignment(
    ...
    try:
        with db.cursor() as cursor:
            cursor.execute(
                "UPDATE assignments SET title = %s, description = %s, "
                "category = %s, due_date = %s, completed = %s, grade = %s "
                "WHERE id = %s",
                (
                    assignment.title,
                    assignment.description,
